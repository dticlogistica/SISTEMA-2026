...
class InventoryService {
    ...
    this.fetchPromise = (async () => {
      ...
      try {
        ...
        if (!url || url.includes('COLE_SUA_URL_AQUI')) {
          ...
          if (!this.dataLoaded) {
             this.processData({ users: [], products: [], movements: [], nes: [], catalog: [] }, true);
             this.dataLoaded = true;
          }
  ...
  async saveCatalogItem(item: CatalogItem): Promise<boolean> {
    const result = await this.postData('saveCatalog', item);
    return result && result.success;
  }

  async deleteCatalogItem(item: CatalogItem): Promise<boolean> {
    const result = await this.postData('deleteCatalog', item);
    return result && result.success;
  }
  ...
  async getCurrentUser(): Promise<User> {
    ...
    if (!this.dataLoaded) { try { this.refreshData(); } catch (e) {} }
  ...
  async changeOwnPassword(oldPasswordPlain: string, newPasswordPlain: string): Promise<{ success: boolean; message?: string }> {
    ...
    if (!isOldPasswordCorrect) return { success: false, message: 'A senha atual está incorreta.' };
    const newHash = await this.hashPassword(newPasswordPlain);
    try {
        const url = this.getApiUrl();
        const userToUpdate = { ...userInDb, password: newHash };
        const response = await fetch(`${url}?action=saveUser`, {
            method: 'POST', redirect: 'follow', credentials: 'omit',
            headers: { 'Content-Type': 'text/plain;charset=utf-8' },
            body: JSON.stringify({ action: 'saveUser', payload: userToUpdate })
        });
        const result = await response.json();
        if (result.success) { await this.refreshData(); return { success: true }; }
        else { return { success: false, message: result.error || 'Erro ao salvar.' }; }
  ...
  async getDashboardStats(): Promise<DashboardStats> {
    ...
    const now = new Date();
    this.cachedMovements.filter(m => m.type === MovementType.EXIT && !m.isReversed).forEach(m => {
        const date = new Date(m.date);
        const key = date.toLocaleString('pt-BR', { month: 'short' });
        monthlyData.set(key, (monthlyData.get(key) || 0) + m.value);
        if (date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear()) { currentMonthOutflow += m.value; }
    });
    const monthlyOutflow = Array.from(monthlyData.entries()).map(([month, value]) => ({ month, value }));
    const topProducts = this.cachedProducts.map(p => ({ name: p.name, value: p.initialQty - p.currentBalance })).sort((a, b) => b.value - a.value).slice(0, 5);
    ...
    return { totalValueStock: totalValue, totalItems: this.cachedProducts.length, lowStockCount: lowStock, monthlyOutflow, topProducts, currentMonthOutflow, criticalItems };
  }
  ...
  async getConsolidatedStock(): Promise<{ name: string; totalBalance: number; unit: string }[]> {
    ...
    this.cachedProducts.forEach(p => {
      if (!map.has(p.name)) { map.set(p.name, { name: p.name, totalBalance: 0, unit: p.unit }); }
  ...
  async calculateDistribution(productName: string, quantityRequested: number): Promise<{ itemsToDeduct: { productId: string; neId: string; qty: number; unitValue: number }[], remainingQty: number }> {
    await this.fetchAllData();
    const availableBatches = this.cachedProducts.filter(p => p.name === productName && p.currentBalance > 0).sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    ...
    for (const batch of availableBatches) {
      ...
      itemsToDeduct.push({ productId: batch.id, neId: batch.neId, qty: take, unitValue: batch.unitValue });
      qtyToDistribute -= take;
    }
    return { itemsToDeduct, remainingQty: qtyToDistribute };
  }
  ...
  async executeDistribution(movementsAllocated: { productId: string; neId: string; qty: number; unitValue: number }[], userEmail: string, obs: string): Promise<{ success: boolean, receiptId?: string }> {
    ...
    const movementsData = movementsAllocated.map(m => {
      ...
      return {
        id: `MOV-${Math.floor(Math.random() * 100000)}`, date: timestamp, type: MovementType.EXIT,
        neId: m.neId, productId: m.productId, productName: product ? product.name : 'Unknown',
        quantity: m.qty, value: m.qty * m.unitValue, userEmail, observation: obs, isReversed: false
      };
    });
  ...
  async reverseMovement(movementId: string, userEmail: string): Promise<boolean> {
    ...
    const reversalMovement: Movement = {
      id: `REV-${Math.floor(Math.random() * 100000)}`, date: new Date().toISOString(), type: MovementType.REVERSAL,
      neId: movement.neId, productId: movement.productId, productName: movement.productName,
      quantity: movement.quantity, value: movement.value, userEmail: userEmail,
      observation: `ESTORNO referente à saída: ${movement.id}`, isReversed: false
    };
  ...
  async createNotaEmpenho(neData: { number: string, supplier: string, date: string }, items: any[]): Promise<boolean> {
    ...
    const newNE: NotaEmpenho = { id: neData.number, supplier: neData.supplier, date: neData.date, status: NEStatus.OPEN, totalValue: totalValue };
    const currentUser = await this.getCurrentUser();
    ...
    items.forEach((item, index) => {
      ...
      const newProduct: Product = {
        id: productId, neId: neData.number, name: item.name, unit: item.unit,
        qtyPerPackage: item.qtyPerPackage, initialQty: item.initialQty, unitValue: item.unitValue,
        currentBalance: item.initialQty, minStock: item.minStock, createdAt: timestamp
      };
      productsPayload.push(newProduct);
      const newMovement: Movement = {
        id: `MOV-IN-${Date.now()}-${index}`, date: timestamp, type: MovementType.ENTRY, neId: neData.number,
        productId: productId, productName: item.name, quantity: item.initialQty, value: item.initialQty * item.unitValue,
        userEmail: currentUser?.email || 'admin@sys.com', observation: 'Entrada Inicial de Nota de Empenho', isReversed: false
      };
      movementsPayload.push(newMovement);
    });
    const result = await this.postData('createNE', { ne: newNE, items: productsPayload, movements: movementsPayload });
    return result && result.success;
  }
